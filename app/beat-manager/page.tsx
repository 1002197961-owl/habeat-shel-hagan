...
import { RhythmPractice }  from '@/components/beat-manager/RhythmPractice'
import { BRAND }           from '@/lib/constants'
import { fmt }             from '@/lib/utils'
import { MOCK_SONGS, PILOT_STATIONS } from '@/lib/mockData'

type Step = 'practice' | 'song-select' | 'rhythm-practice' | 'instrument-select'
...
export default function BeatManagerPage() {
  ...
  const [countdown,  setDown]   = useState(DEFAULT_SECS)
  const [elapsed,    setUp]     = useState(0)
  const [running,    setRun]    = useState(false)
  const [songsCount, setSongsC] = useState(3)
  const [log,        setLog]    = useState(['10:00 — נפתחה פעילות הביט 🎵','09:55 — כל הילדים בתחנות ✅'])

  useEffect(() => {
    if (!running) return
    const d = setInterval(() => setDown(s => s>0?s-1:0), 1000)
    const u = setInterval(() => setUp(e => e+1), 1000)
    return () => { clearInterval(d); clearInterval(u) }
  }, [running])

  const switchStation = useCallback(() => {
    const next = (stationIdx+1) % STATION_NAMES.length
    setStation(next); setSongsC(s=>s+1)
  ...
  if (step === 'song-select') return (
    <AppShell bg="#f0f9ff">
      <BackHeader title="בחירת שיר 🎵" bg={BRAND.cyan} backHref="#" />
      <div className="p-4 space-y-3">
        <div style={{fontSize:13,color:'#6b7280',fontWeight:600}}>בחרי שיר לפעילות היום:</div>
        {MOCK_SONGS.map((song,i) => (
          <motion.div key={song.id} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} transition={{delay:i*0.07}} whileTap={{scale:0.97}}>
            <Card style={{cursor:'pointer',display:'flex',alignItems:'center',gap:14,
              border:'2px solid '+(songId===song.id?song.color:'transparent'),
cat > app/beat-manager/page.tsx << 'ENDOFFILE'
'use client'
import { useEffect, useState, useCallback } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { AppShell }        from '@/components/layout/AppShell'
...
export default function BeatManagerPage() {
        ...
        {MOCK_SONGS.map((song,i) => (
          <motion.div key={song.id} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} transition={{delay:i*0.07}} whileTap={{scale:0.97}}>
            <Card style={{cursor:'pointer',display:'flex',alignItems:'center',gap:14,
              ...
              onClick={()=>{setSongId(song.id);setStep('rhythm-practice')}}>
  ...
  if (step === 'rhythm-practice') return (
    <AppShell bg="#f0f9ff">
      <BackHeader title="תרגול קצב 🎶" bg={BRAND.cyan} backHref="#" />
      <div className="p-4 space-y-3">
        <Card style={{background:activeSong.color+'12',border:'2px solid '+activeSong.color+'33'}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <span style={{fontSize:30}}>{activeSong.emoji}</span>
            <div style={{flex:1}}>
              <div className="font-black" style={{fontSize:16,color:BRAND.navy}}>{activeSong.title}</div>
              <div style={{fontSize:12,color:'#9ca3af',marginBottom:5}}>{activeSong.artist} · {activeSong.duration}</div>
              {activeSong.difficulty && <DifficultyBadge difficulty={activeSong.difficulty}/>}
            </div>
            <button onClick={()=>setStep('song-select')} style={{background:'none',border:'none',cursor:'pointer',color:BRAND.cyan,fontWeight:700,fontSize:13,fontFamily:'inherit'}}>שנה</button>
          </div>
        </Card>
        <Card>
          <SectionHeader title="תרגול קצב" emoji="🥁" subtitle="עקבי אחרי הצבעים"/>
          <RhythmPractice song={activeSong} onContinue={()=>setStep('instrument-select')}/>
        </Card>
      </div>
    </AppShell>
  )
  ...
  if (step === 'instrument-select') {
    const toggle = (id: string) => setInstr(arr=>arr.includes(id)?arr.filter(x=>x!==id):[...arr,id])
    return (
      <AppShell bg="#f0f9ff">
        <BackHeader title="בחירת כלים 🎸" bg={BRAND.cyan} backHref="#"/>
        <div className="p-4 space-y-3">
          <Card style={{background:activeSong.color+'14',border:'2px solid '+activeSong.color+'33',display:'flex',alignItems:'center',gap:10}}>
          ...
          <Btn full bg={'linear-gradient(135deg,'+BRAND.cyan+',#0096c7)'}
            onClick={()=>{
              ...
              setStep('practice')
            }} style={{padding:'15px',fontSize:17,borderRadius:16}}>▶ התחל פעילות!</Btn>
          <Btn bg="#e5e7eb" full onClick={()=>setStep('rhythm-practice')} style={{color:'#6b7280',padding:'11px',fontSize:14}}>← חזרה לתרגול קצב</Btn>
        </div>
      </AppShell>
    )
  }
    ...
    <AppShell bg="#f0f9ff">
      ...
      <div className="p-4 space-y-3">
        <motion.div className="flex gap-3" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.04}}>
          ...
          <StatBadge value={songsCount} label="שירים" emoji="🎵" color={BRAND.yellow}/>
        ...
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.07}}>
          <Card style={{background:activeSong.color+'12',display:'flex',alignItems:'center',gap:12,border:'2px solid '+activeSong.color+'30'}}>
            <span style={{fontSize:28}}>{activeSong.emoji}</span>
            <div style={{flex:1}}>
              ...
              <div style={{display:'flex',gap:5,marginTop:4,flexWrap:'wrap',alignItems:'center'}}>
                ...
                {activeSong.difficulty && <span style={{marginRight:4}}><DifficultyBadge difficulty={activeSong.difficulty}/></span>}
              </div>
            </div>
            <button onClick={()=>setStep('song-select')} style={{background:BRAND.cyan,border:'none',borderRadius:10,padding:'6px 12px',color:'white',fontWeight:800,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>שנה</button>
          </Card>
        </motion.div>
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1}}>
          <Card style={{background:'linear-gradient(135deg,'+BRAND.navy+',#312e81)'}} className="text-center py-5">
            <AnimatePresence>{running&&<motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{position:'absolute',inset:0,borderRadius:20,pointerEvents:'none',boxShadow:'inset 0 0 60px '+BRAND.cyan+'22'}}/>}</AnimatePresence>
            <div className="relative inline-flex items-center justify-center" style={{width:140,height:140}}>
              <svg width="140" height="140" className="absolute" style={{transform:'rotate(-90deg)'}}>
                ...
                <circle cx="70" cy="70" r={R} fill="none" stroke={running?BRAND.cyan:'rgba(255,255,255,0.25)'} strokeWidth="9" strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={circ*(1-pct)} style={{transition:'stroke-dashoffset 0.6s ease,stroke 0.4s'}}/>
              </svg>
              <div>
            ...
            <div className="flex justify-center mt-2 mb-4"><WaveBar active={running} count={12} color={running?BRAND.cyan:undefined} height={20}/></div>
            <div className="flex justify-center gap-3">
              <Btn bg={running?'#ef4444':BRAND.green} onClick={()=>setRun(r=>!r)} style={{minWidth:130,fontSize:16,padding:'11px 22px'}}>{running?'⏸ עצור':'▶ התחל'}</Btn>
              <button onClick={()=>{setDown(DEFAULT_SECS);setUp(0);setRun(false)}} style={{width:44,height:44,background:'rgba(255,255,255,0.13)',border:'none',borderRadius:13,color:'white',fontSize:20,cursor:'pointer'}}>↺</button>
            </div>
          </Card>
        </motion.div>
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.16}}>
          <Card style={{background:activeStation.color+'1a',border:'2px solid '+activeStation.color+'44'}}>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={stationIdx} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} exit={{opacity:0,x:12}} transition={{duration:0.2}} className="flex items-center gap-3 mb-3">
                <motion.span style={{fontSize:36}} initial={{scale:0.7}} animate={{scale:1}} transition={{type:'spring',stiffness:300,damping:18}}>{activeStation.emoji}</motion.span>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.22}}>
          <Card>
            ...
            <AnimatePresence initial={false}>
              {log.map((entry,i)=>(
                <motion.div key={entry+i} initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}}
                  style={{fontSize:13,color:'#374151',fontWeight:500,overflow:'hidden',paddingBlock:9,borderBottom:i<log.length-1?'1px solid #f3f4f6':'none'}}>
                  {entry}
                </motion.div>
              ))}
            </AnimatePresence>
